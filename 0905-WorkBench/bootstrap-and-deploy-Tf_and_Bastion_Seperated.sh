...
## 테라폼 실행부분 
## -upgrade 옵션: 사용 가능한 최신 버전의 제공자 플러그인과 모듈을 다운로드하여 설치함
## -auto-approve 옵션: 사용자 확인 없이 자동으로 적용함
## -chdir 옵션: 지정된 디렉토리로 변경한 후 명령어를 실행함
## terraform init: 테라폼 작업 디렉토리를 초기화하고 필요한 플러그인과 모듈을 다운로드함
## terraform apply: 테라폼 구성을 적용하여 인프라를 생성, 변경 또는 삭제함
