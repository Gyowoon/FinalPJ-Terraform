...
# ---- EKS metadata & auth (no external aws cli needed) ----
data "aws_eks_cluster" "eks" {
  # 이미 이 파일 안에서 생성한 클러스터 리소스명이 aws_eks_cluster.eks 라고 가정
  # (만약 이름이 다르면 .eks 부분만 당신의 리소스명으로 바꾸세요)
...
provider "kubernetes" {
  ...
  # config_path = null  # (선택) kubeconfig 사용 무시
}
# ----------------------------- Main Infrastructure -----------------------------
...
resource "aws_eks_cluster" "eks" {
  ...
  # kubeconfig 자동 갱신 추가 08/27-Edited (09/05 주석처리)
...
data "aws_ami" "bastion" {
  ...
  filter {
    name   = "name"
    values = ["al2023-ami-*-x86_64"] #values = ["al2023-ami-minimal-*"]
  }
  filter {
    name   = "architecture"
    values = ["x86_64"]
...
###################################################################
# Route53 및 ExternalDNS 설정 (09/03 새로 추가된 부분)               Start
###################################################################
# ---------------------------------------------------------------------------
# Route53 설정 (09/03 새로 추가된 부분)
...
# ----------------------------------------------------------------------------
# ExternalDNS용(AWS Route53) IAM Role 및 Role Policy Attachment (09/03 새로 추가된 부분)
...
# ----------------------------------------------------------------------------
# ExternalDNS용(AWS Route53) Pod Identity Association (09/03 새로 추가된 부분)
resource "aws_eks_pod_identity_association" "external_dns" {
  cluster_name    = aws_eks_cluster.eks.name ##var.cluster_name
  namespace       = "kube-system"
  service_account = "external-dns"
  role_arn        = aws_iam_role.external_dns.arn
    depends_on = [
    aws_eks_cluster.eks,                 # 클러스터 먼저
    aws_iam_role.external_dns,           # Role/Policy 먼저
    aws_eks_node_group.nodes,            # (선택) 노드까지 준비되면 더 안정적
  ]
}
# ----------------------------------------------------------------------------
###################################################################
# Route53 및 ExternalDNS 설정 (09/03 새로 추가된 부분)               End
###################################################################